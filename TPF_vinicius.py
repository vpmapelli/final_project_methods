...
line_plt_style = 'k+'        #estilo da linha do resultado pelo fem
...
rho = U[0:N_final*gdl:gdl]
plt.plot(x,rho,line_plt_style)
plt.xlim(x_axis)
...
plt.title('Velocity')
plt.grid(1,'major','both')
v = U[1:N_final*gdl+1:gdl]/rho
plt.plot(x,v,line_plt_style)
plt.xlim(x_axis)
...
plt.title('Pressure')
plt.grid(1,'major','both')
P = (gamma-1)*(U[2:N_final*gdl+2:gdl] - rho*v*v/2)
plt.plot(x,P,line_plt_style)
plt.xlim(x_axis)
...
c_analy = np.sqrt(gamma*P_analy/rho_analy)
print("Courant number analy:", max(np.abs(u_analy)+c_analy)*delta_t/(x[1]-x[0]))

############################# PLOTANDO DENSIDADE EM X,T ###########################
